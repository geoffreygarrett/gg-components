...
mod presence {
    ...
    pub fn Presence(
        ...
        children: TypedChildrenFn<impl IntoView + 'static>) -> impl IntoView {
...
pub fn Tabs(
    ...
    let orientation = orientation.get().unwrap_or_else(|| "horizontal".to_string());
    let dir = dir.get().unwrap_or_else(|| "ltr".to_string());
    let activation_mode = activation_mode.get().unwrap_or_else(|| "automatic".to_string());
...
pub fn TabsList(
    ...
    let TabsContextValue { orientation, dir, .. } = use_tabs_context(TAB_LIST_NAME);
    let children = StoredValue::new(children.into_inner());
    view! {
...
pub fn TabsTrigger(
    ...
    let TabsContextValue { selected, base_id, on_value_change, .. } = use_tabs_context(TAB_TRIGGER_NAME);
    // Check if selected
    let value = StoredValue::new(value);
    let children = StoredValue::new(children.into_inner());
    let is_selected = Signal::derive(move || {
        selected.get().as_ref().map(|s| s == &value.with_value(|v| v.clone())).unwrap_or(false)
    });
...
pub fn TabsContent(
    ...
    let TabsContextValue { selected, orientation, dir: _, base_id, .. } = use_tabs_context(TAB_CONTENT_NAME);
    let value = StoredValue::new(value);
    let children = StoredValue::new(children.into_inner());
    ...
    let is_selected = Signal::derive(move || {
        selected.get().map_or(false, |s| s == value.with_value(|v|v.clone()))
    });
