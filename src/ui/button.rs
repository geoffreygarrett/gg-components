...
impl From<&str> for ButtonVariant {
    fn from(variant: &str) -> Self {
        match variant.to_lowercase().as_str() {
            ...
            // Default to the Default variant if string doesn't match
...
impl From<String> for ButtonSize {
    fn from(size: String) -> Self {
        ButtonSize::from(size.as_str())
    }
}

// Implement for String by leveraging the &str implementation
impl From<String> for ButtonVariant {
    fn from(variant: String) -> Self {
        ButtonVariant::from(variant.as_str())
    }
}
