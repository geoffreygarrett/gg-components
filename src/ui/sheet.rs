...
pub fn Sheet(
    #[prop(into, optional)] open: Option<RwSignal<bool>>,
...
pub fn SheetHeader(
    ...
    view! {
        <div class=cn!(
            "flex flex-col space-y-2 text-center sm:text-left", class.get()
        )>{children()}</div>
    }
}
...
pub fn SheetFooter(
    ...
    view! {
        <div class=cn!(
            "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2", class.get()
        )>{children()}</div>
    }
}
...
pub fn SheetTitle(
    ...
    view! { <h2 class=cn!("text-lg font-semibold text-foreground", class.get())>{children()}</h2> }
}
...
pub fn SheetDescription(
    ...
    view! { <p class=cn!("text-sm text-muted-foreground", class.get())>{children()}</p> }
}
